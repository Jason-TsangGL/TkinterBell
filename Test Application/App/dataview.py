...
class Application(Frame):
    ...
    def create_widgets(self):
        self.winfo_toplevel().title("MRI Viewer")
        self.l1 = Label(self.master, text="Input Image")
        self.l2 = Label(self.master, text="Output Image")
        self.l3 = Label(self.master, text="Predicted Segmentations")
        ...
        self.btn= Button(self.master,text="Show Image Overlay", style="D.TButton")
        self.btn["command"] = self.show_graph
        self.btn.grid(row=4,column=0,sticky=W)

        self.stats= Button(self.master,text="Next MR Slice", style="D.TButton")
        self.stats["command"] = self.show_stats
        self.stats.grid(row=4,column=1,sticky=W)

        self.quit= Button(self.master,text="Exit Session", style="D.TButton",command=self.master.destroy)
        self.quit.grid(row=4,column=1,sticky=E)

    def show_graph(self):
        print("Loading Original MR Image")
